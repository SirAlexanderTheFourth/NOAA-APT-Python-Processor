...
def fm_demodulate(rate, data):
    ...
    for i in range(0, len(data), chunk_size):
        ...
        # Calculate the phase difference of consecutive samples
        phase_diff = np.diff(np.angle(chunk))
        # Unwrap the phase differences to prevent discontinuities
        phase_diff_unwrapped = np.unwrap(phase_diff)
        # Scale the phase differences to get the demodulated signal
        demodulated_chunk = phase_diff_unwrapped * (rate / (2 * np.pi))
...
def receive_and_process_pass(satellite_name, frequency, tle1, tle2):
    ...
    data_demodulated = fm_demodulate(sdr.sample_rate, data)  # Demodulate the data
